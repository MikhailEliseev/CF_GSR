...
def create_app():
    ...
    with app.app_context():
        ...
        for module_name in modules:
            ...
            if not existing:
                ...
                # Добавляем дефолтную Google Sheets ссылку для модуля вакансий
                if module_name == 'vacancies':
                    ...
                    settings.set_additional_settings(additional_settings)
    ...
    @app.route('/api/settings/<module_name>/upload-redpolicy', methods=['POST'])
    def upload_redpolicy(module_name):
        """Загрузка PDF файла редполитики"""
        if module_name not in ['trends', 'vacancies', 'experts']:
            return jsonify({'success': False, 'message': 'Модуль не найден'}), 404
        ...
        if not file.filename.endswith('.pdf'):
            return jsonify({'success': False, 'message': 'Только PDF файлы'}), 400
        
        try:
            # Создать папку если не существует
            redpolicy_dir = os.path.join('static', 'redpolicy')
            os.makedirs(redpolicy_dir, exist_ok=True)
            
            # Сохранить файл
            filename = f"{module_name}_redpolicy.pdf"
            filepath = os.path.join(redpolicy_dir, filename)
            file.save(filepath)
            
            # Обновить настройки
            settings = _get_settings(module_name)
            if settings:
                settings.redpolicy_pdf_path = filepath
                db.session.commit()
                return jsonify({'success': True, 'filepath': filepath})
            
            return jsonify({'success': False, 'message': 'Настройки не найдены'}), 404
    ...
    # Эндпоинт /api/trends/rewrite теперь в routes/trends.py
    ...
    @app.route('/api/vacancies/upload-csv', methods=['POST'])
    def upload_csv_file():
        ...
        try:
            ...
            print(f"Загружен CSV файл, размер: {len(csv_data)} символов")
            
            # Используем прямой парсинг
            vacancies = parse_vacancies_direct(csv_data)
            
            print(f"Успешно распарсено {len(vacancies)} вакансий")
            return jsonify({
                "success": True,
                "vacancies": vacancies,
                "count": len(vacancies)
    ...
    def generate_vacancy_audio():
        try:
            print(f"🎵 === НАЧАЛО generate_vacancy_audio ===")
            data = request.get_json() or {}
            text = (data.get('text') or '').strip()
            ...
            voice_id = data.get('voice_id') or additional.get('default_voice_id') or 'CfPkL4eEqBDVYldLZuY5'
            model_id = data.get('model_id') or additional.get('default_voice_model') or 'eleven_monolingual_v1'
            ...
            eleven_key = _get_api_key('elevenlabs_api_key')
            print(f"🔑 ElevenLabs ключ: {eleven_key[:20]}..." if eleven_key else "❌ Ключ не найден")
            if not eleven_key:
                return jsonify({'success': False, 'message': 'ElevenLabs API ключ не настроен'}), 400

            from api.elevenlabs_simple import ElevenLabsSimple
            client = ElevenLabsSimple(eleven_key)
            print(f"🎵 Создан клиент ElevenLabs для текста: {text[:50]}...")
            ...
            print(f"🎵 === КОНЕЦ generate_vacancy_audio ===")
            ...
            print(f"❌ Ошибка генерации аудио вакансии: {e}")
            return jsonify({'success': False, 'message': f'Ошибка создания аудио: {str(e)}'}), 500
    ...
    from routes.vacancies import vacancies_bp, parse_vacancies_direct
    app.register_blueprint(trends_bp)
    app.register_blueprint(vacancies_bp)
    print(f"✅ Blueprint 'trends' registered with {len([rule for rule in app.url_map.iter_rules() if 'trends' in str(rule)])} routes")
    print(f"✅ Blueprint 'vacancies' registered with {len([rule for rule in app.url_map.iter_rules() if 'vacancies' in str(rule)])} routes")
...
def list_vacancy_avatars():
    ...
    try:
        heygen_key = _get_api_key('heygen_api_key', ['vacancies', 'trends'])
        if not heygen_key:
            return jsonify({'success': False, 'message': 'HeyGen API ключ не настроен', 'avatars': _default_avatars()}), 200
        ...
        print(f"❌ Ошибка получения аватаров для вакансий: {e}")
        return jsonify({'success': False, 'message': f'Ошибка: {str(e)}', 'avatars': _default_avatars()}), 200

if __name__ == '__main__':
    app = create_app()
    if isinstance(app, tuple):
        app = app[0]  # Берем первый элемент из tuple
