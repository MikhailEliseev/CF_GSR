...
class ElevenLabsSimple:
    ...
    def text_to_speech_advanced(self, text: str, voice_id: str = "jP9L6ZC55cz5mmx4ZpCk", 
        ...
        """Преобразование текста в речь с полными настройками голоса"""
        try:
            print(f"🎵 Генерация аудио: {text[:50]}...")
            print(f"⚙️ Настройки: voice={voice_id}, model={model_id}, speed={speed}, stability={stability}, similarity={similarity_boost}")
    ...
    def generate_audio_advanced(self, text: str, voice_id: str = "jP9L6ZC55cz5mmx4ZpCk", 
        ...
        """Полный пайплайн с полными настройками голоса"""
        print(f"🎵 Генерация аудио для текста: {text[:50]}...")
        ...
        try:
            audio_data = self.text_to_speech_advanced(text, voice_id, model_id, speed, stability, similarity_boost)
            if not audio_data:
                print("❌ Не удалось сгенерировать аудио")
                return self._create_audio_placeholder(text)
