#!/usr/bin/env python3
"""
Правильное добавление endpoint для генерации аудио
"""
...
def add_audio_endpoint_correct():
    """Правильно добавляем endpoint для генерации аудио"""
    print("🔧 Правильно добавляем endpoint для генерации аудио...")
    ...
    try:
        # 1. Создаем бекап
        print("\n1️⃣ Создаем бекап...")
        stdin, stdout, stderr = ssh.exec_command("cp /root/routes/vacancies.py /root/routes/vacancies.py.backup_final")
        stdout.read()
        print("✅ Бекап создан")
        
        # 2. Добавляем правильный endpoint в конец файла
        print("\n2️⃣ Добавляем правильный endpoint...")
        
        # Создаем временный файл с endpoint
        endpoint_code = '''
...
'''
        ...
        # Записываем endpoint в файл
        stdin, stdout, stderr = ssh.exec_command(f"cat > /tmp/audio_endpoint.py << 'EOF'{endpoint_code}\nEOF")
        stdout.read()
        
        # Добавляем endpoint в конец файла
        stdin, stdout, stderr = ssh.exec_command("cat /tmp/audio_endpoint.py >> /root/routes/vacancies.py")
        stdout.read()
...
if __name__ == "__main__":
    success = add_audio_endpoint_correct()
    if success:
        print("\n✅ Endpoint для генерации аудио добавлен и работает!")
        print("Теперь можно тестировать на http://72.56.66.228/module/vacancies")
        print("Система больше не застрянет на генерации аудио!")
    else:
        print("\n❌ Проблема не решена")
