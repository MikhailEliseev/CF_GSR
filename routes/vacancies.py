...
""" + csv_data[:8000]  # Ограничиваем размер для API
        ...
        # Получаем API ключ из config.py
        openai_key = _get_api_key('openai_api_key')
        if not openai_key:
            return jsonify({
                "success": False,
                "error": "OpenAI API ключ не найден в config.py"
            }), 500
...
def generate_vacancy_text():
    ...
    try:
        ...
        # Используем дефолтный master prompt
        master_prompt = "Создай контент для вакансии на основе предоставленных данных."
...
def generate_video():
    ...
    try:
        # Получаем API ключ HeyGen из config.py
        heygen_key = _get_api_key('heygen_api_key')
        if not heygen_key:
        ...
        # Используем переданный avatar_id или дефолтный
...
def generate_submagic():
    ...
    try:
        # Получаем API ключ Submagic из config.py
        submagic_key = _get_api_key('submagic_api_key')
        if not submagic_key:
...
@vacancies_bp.route('/api/vacancies/list-avatars', methods=['GET'])
def list_vacancy_avatars():
    """Получение списка доступных аватаров HeyGen для модуля вакансий"""
    try:
        heygen_key = _get_api_key('heygen_api_key')
        if not heygen_key:
            return jsonify({'success': False, 'message': 'HeyGen API ключ не настроен', 'avatars': []}), 200
        ...
        return jsonify({'success': False, 'message': f'Ошибка: {str(e)}', 'avatars': []}), 200
