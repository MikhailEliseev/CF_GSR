...
from services import OpenAIService, ElevenLabsService, HeyGenService
...
class BaseModule(ABC):
    ...
    def __init__(self, module_name: str):
        ...
        # Инициализация сервисов и клиентов
    ...
    def _init_api_clients(self):
        ...
        self.elevenlabs_service = ElevenLabsService(elevenlabs_key)

        # HeyGen сервис
        heygen_key = api_keys.get('heygen_api_key')
        self.heygen_service = HeyGenService(heygen_key)
    ...
    def generate_text_content(self, context: str) -> Optional[str]:
        ...
        return self.openai_service.generate_with_assistant(
            self.settings.openai_assistant_id,
            self.settings.master_prompt,
            context
        )

    def generate_speech(self, text: str, voice_id: str, model_id: Optional[str] = None) -> Optional[str]:
        """
        ...
        return self.elevenlabs_service.generate_audio(text, voice_id=voice_id, model_id=model_id)
    ...
    def generate_video(self, audio_url: str, avatar_id: str) -> Optional[str]:
        ...
        video_result = self.heygen_service.generate_video(audio_url, avatar_id)
        if isinstance(video_result, dict):
            return video_result.get('video_url') or video_result.get('video_id')
        return video_result
    ...
    def execute_full_pipeline(self, task_id: str, context: str, voice_id: str, avatar_id: str,
        ...
        try:
            ...
            audio_url = self.generate_speech(text, voice_id, model_id=model_id)
            if not audio_url:
