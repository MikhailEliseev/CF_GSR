    ...
    def scrape_user_posts(self, username: str, count: int = 20,
        ...
        for attempt in range(max_retries):
            try:
                ...
                wait_interval = 10  # Проверяем каждые 10 секунд
                waited_time = 0
                ...
                while waited_time < max_wait_time:
                    ...
                    if status in ['SUCCEEDED', 'FINISHED']:
                        ...
                        raise Exception(f"Актор завершился с ошибкой: {status}")
                    
                    time.sleep(wait_interval)
                    waited_time += wait_interval
                
                if waited_time >= max_wait_time:
                    raise Exception(f"Таймаут ожидания актора: {max_wait_time}с")
