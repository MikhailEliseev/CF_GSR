#!/usr/bin/env python3
"""
Создание пакета для развертывания на сервере 72.56.66.228
"""
...
def create_deployment_package():
    ...
    archive_name = f"gsr_deploy_{timestamp}.tar.gz"
    ...
    temp_dir = Path(f"temp_deploy_{timestamp}")
    ...
    instructions = f"""# 🚀 ИНСТРУКЦИИ ПО РАЗВЕРТЫВАНИЮ НА СЕРВЕРЕ
...
"""
    ...
    with open(temp_dir / "DEPLOY_INSTRUCTIONS.md", 'w', encoding='utf-8') as f:
        f.write(instructions)
    
    print("   ✅ Созданы инструкции: DEPLOY_INSTRUCTIONS.md")
    
    # Создаем архив
    print(f"\n📦 Создаем архив {archive_name}...")
    with tarfile.open(archive_name, "w:gz") as tar:
        tar.add(temp_dir, arcname="gsr_deploy")
...
def create_upload_script(archive_name):
    """Создание скрипта для загрузки на сервер"""
    print(f"\n📤 СОЗДАНИЕ СКРИПТА ЗАГРУЗКИ")
    print("=" * 50)
    
    script_name = f"upload_{archive_name.replace('.tar.gz', '')}.sh"
    
    script_content = f"""#!/bin/bash
# Скрипт для загрузки {archive_name} на сервер 72.56.66.228

echo "🚀 ЗАГРУЗКА НА СЕРВЕР 72.56.66.228"
echo "=================================="
...
ssh root@72.66.66.228 "cd /var/www/gsr && pkill -f 'python.*app.py' || true"
...
def main():
    """Основная функция"""
    print("🚀 СОЗДАНИЕ ПАКЕТА ДЛЯ РАЗВЕРТЫВАНИЯ НА СЕРВЕРЕ")
    print("=" * 60)
    
    # Проверяем, что мы в правильной директории
    if not Path("app_for_server_final.py").exists():
        print("❌ app_for_server_final.py не найден")
        print("🔧 Сначала запустите: python3 fix_local_versions.py")
        return
    ...
    script_name = create_upload_script(archive_name)
    
    print("\n" + "=" * 60)
    print("📊 ИТОГОВЫЙ СТАТУС")
    print("=" * 60)
    
    print("🎉 ПАКЕТ ДЛЯ РАЗВЕРТЫВАНИЯ ГОТОВ!")
    print(f"📦 Архив: {archive_name}")
    print(f"📤 Скрипт загрузки: {script_name}")
    print(f"📋 Инструкции: DEPLOY_INSTRUCTIONS.md")
    ...
    print(f"   Смотрите DEPLOY_INSTRUCTIONS.md")
