# ИСПРАВЛЕННЫЙ ФАЙЛ ДЛЯ СЕРВЕРА
# Замените содержимое /root/routes/vacancies.py на этот код
# ================================================
    ...
    try:
        ...
        for i, row in enumerate(reader):
            try:
                ...
                # Извлекаем все поля с ПРАВИЛЬНЫМИ индексами (исправлено)
                vacancy_data = {
                    'position': row[1].strip() if len(row) > 1 else '',  # B - Объект/Название вакансии
                    'location': row[1].strip() if len(row) > 1 else '',    # B - Объект (используется как локация)
                ...
                # Проверяем минимальную длину названия вакансии
                if len(vacancy_data['position'].strip()) < 3:
                    validation_errors.append("Название вакансии слишком короткое")
                
                # Проверяем, что salary содержит числа или валюту
                salary = vacancy_data['salary'].strip()
                if salary and not any(char.isdigit() for char in salary):
                    validation_errors.append("Зарплата не содержит чисел")
                
                # Проверяем positions_needed
                positions = vacancy_data['positions_needed'].strip()
                if positions and positions != '0':
                    # Извлекаем число из строки типа "15 муж." или "5 чел."
                    import re
                    numbers = re.findall(r'\d+', positions)
                    if not numbers:
                        validation_errors.append("Потребность не содержит чисел")
                
                if validation_errors:
                    logger.warning(f"Строка {i}: Ошибки валидации - {validation_errors}")
                    # Не добавляем вакансию с ошибками валидации
                    continue
                
                # Логируем каждую обработанную вакансию
                logger.info(f"Строка {i}: Обрабатываем вакансию '{vacancy_data['position'][:30]}...'")
                logger.debug(f"Строка {i}: Полные данные - {vacancy_data}")
