...
// GSRPipeline - глобальный объект для управления статусами и шагами
window.GSRPipeline = {
    showStatus: function(elementId, type, message) {
        ...
        element.innerHTML = `<div class="alert ${className} mb-0">${icon}${message}</div>`;
    ...
    activateStep: function(stepId, options = {}) {
        const step = document.getElementById(stepId);
        if (step) {
            step.classList.add('step-active');
            if (options.exclusive) {
                // Деактивировать другие шаги
